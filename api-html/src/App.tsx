      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-xl p-8">
        ...
        {cargando && (
          <div className="flex flex-col items-center justify-center py-12 animate-pulse">
            ...
            <p className="text-gray-600 font-medium">Obteniendo productos desde la API...</p>
          </div>
        )}
        ...
        {error && (
          <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded-md my-4 text-center">
            ...
            <p className="text-xs text-gray-400 mt-2">Revisa si tu contenedor Docker está activo en el puerto 8000.</p>
          </div>
        )}
        ...
        {!cargando && !error && productos.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
            <p className="text-gray-500 font-medium">No hay productos registrados en la base de datos.</p>
          </div>
        )}
        ...
        {!cargando && !error && productos.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {productos.map((producto:{id:number, nombre:string, precio:number, descripcion: string, categoria:string}, index) => (
              <div 
        ...
        <footer className="mt-8 pt-4 border-t border-gray-100 text-center text-xs text-gray-400">
          Consumiendo: <code className="bg-gray-100 px-1 py-0.5 rounded text-red-500">{API_URL}</code>
        </footer>
